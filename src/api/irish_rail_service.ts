...
const IRISH_RAIL_ALL_STATIONS_URL =
  "http://api.irishrail.ie/realtime/realtime.asmx/getAllStationsXML";
const IRISH_RAIL_STATION_DATA_URL =
...
export const fetchAllIrishRailStations = async (): Promise<Station[]> => {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch Irish Rail stations: ${response.statusText}`,
      );
    }
    ...
    const stationsData = Array.isArray(rawStations)
      ? rawStations
      : [rawStations];
    return stationsData.map(
      (station: any): Station => ({
        ...
        type: station.StationDesc.toLowerCase().includes("dart")
          ? "DART"
          : "Train",
...
export const fetchIrishRailForecast = async (
  stationCode: string,
): Promise<Arrival[]> => {
  try {
    const response = await fetch(
      `${IRISH_RAIL_STATION_DATA_URL}${stationCode}`,
    );
    if (!response.ok) {
      throw new Error(
        `Failed to fetch Irish Rail forecast: ${response.statusText}`,
      );
    }
    const xmlData = await response.text();
    const result = xmlParser.parse(xmlData);
    ...
    const arrivalData = Array.isArray(rawArrivals)
      ? rawArrivals
      : [rawArrivals];
    return arrivalData.map(
      (arrival: any): Arrival => ({
        ...
        direction:
          arrival.Direction === "Northbound" ? "Northbound" : "Southbound",
      }),
    );
  } catch (error) {
